...
def run(context, session):
	session.sessionbreakers = [SessionBreak(max_request = 10000)]
	session.queue(Requests('http://www.navishop.nl/index.php'), process_frontpage, {})

def process_frontpage(data, context, session):
	categs = data.xpath('//ul[@id="categories"]/li/a[not(regexp:test(text(), "Accessoires|Autospecifek"))]')
	for cat in categs:
...
def process_category(data, context, session):
	...
	if not products:
		...
		for cat in categs:
			category = cat.xpath('./text()').string(multiple=True)
			url = cat.xpath('@href').string()
			session.queue(Request(url), process_product, dict(context, url=url, category=category))
	else:
